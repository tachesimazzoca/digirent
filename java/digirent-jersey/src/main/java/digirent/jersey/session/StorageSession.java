...
public class StorageSession {
    ...
    public StorageSession(Storage<Map<String, Object>> storage,
                          NewCookie cookie) {
        this(storage, cookie, null);
    }

    public StorageSession(Storage<Map<String, Object>> storage,
                          NewCookie cookie, String sessionId) {
        this.storage = storage;
        this.cookie = cookie;
        // TODO: Add an option like doNotCreate
        if (sessionId == null || sessionId.isEmpty()
                || !storage.read(sessionId).isPresent()) {
            this.sessionId = this.storage.create(new HashMap<String, Object>());
        } else {
